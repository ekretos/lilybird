...
export class ListenerCompiler<C extends MockClient, T extends Transformers<C>> {
    ...
    #createListener(event: GatewayEvent, name: string, extra: { when: CacheExecutionPolicy, content: string } | undefined = undefined): void {
        ...
        if (typeof transformer !== "undefined") {
            ...
            if (typeof extra !== "undefined") {
                ...
                if (extra.when === CacheExecutionPolicy.FIRST) {
                    temp.push(extra.content, typeof handler !== "undefined" ? `await ${name}(${spread}td)` : "");
                } else {
                    temp.push(typeof handler !== "undefined" ? `await ${name}(${spread}td)` : "", extra.content);
                }
    ...
    #addReadyListener(once = false, listener?: (client: C, payload: Ready["d"]) => Awaitable<any>): void {
        const onceListener = this.#callbacks.get("once_ready");
        const readyArr = [];
        readyArr.push("if(payload.t === \"READY\"){", "Object.assign(client_ptr,{user:");
        ...
        if (this.#shouldTransformClientUser && typeof this.#transformers.userUpdate !== "undefined") {
            ...
            readyArr.push("await t_userUpdate(client_ptr, payload.d.user)");
        } else readyArr.push("payload.d.user");

        readyArr.push(`,sessionId:payload.d.session_id,application:payload.d.application,ready:${once ? "false" : "true"}});`, "Object.assign(resume_info_ptr,{url:payload.d.resume_gateway_url,id:payload.d.session_id});");
    ...
    public appendCachingHandlers(options: CachingOptions): this {
        if (typeof options !== "undefined" && options.delegate !== CachingDelegationType.TRANSFORMERS) {
            ...
            const defaults: SelectiveCache = { create: CacheExecutionPolicy.FIRST, update: CacheExecutionPolicy.LAST, delete: CacheExecutionPolicy.LAST };
            ...
            if (typeof enabledGuild?.create !== "undefined") {
                ...
                if (typeof enabledChannel?.create !== "undefined") {
                    if (options.applyTransformers && !this.#callbacks.has("t_channelCreate")) {
                    ...
                    temp.push("for (let i = 0, {channels} = td, {length} = channels; i < length; i++){", `const channel = ${options.applyTransformers ? "t_channelCreate(client_ptr, channels[i])" : "channels[i]"};`, `await client_ptr.cache.set(${CacheElementType.CHANNEL}, channel.id, channel);`, "}");
                }
                if (typeof enabledThreads?.create !== "undefined") {
                    ...
                    temp.push("for (let i = 0, {threads} = td, {length} = threads; i < length; i++){", `const channel = ${options.applyTransformers ? "t_channelCreate(client_ptr, threads[i])" : "threads[i]"};`, `await client_ptr.cache.set(${CacheElementType.CHANNEL}, channel.id, channel);`, "}");
                }
                if (typeof options.enabled.voiceState !== "undefined") {
                    if (options.applyTransformers && !this.#callbacks.has("t_voiceStateUpdate")) {
                    ...
                    this.#createListener(GatewayEvent.VoiceStateUpdate, "voiceStateUpdate", { when: options.enabled.voiceState, content: `await client_ptr.cache.set(${CacheElementType.VOICE_STATE}, ${options.applyTransformers ? `\`\${td.${vUid}}:\${td.${vCid}}\`` : `\`\${payload.d.${vUid}}:\${payload.d.${vCid}}\``}, payload.d);` });
                    temp.push(`for (let i = 0, {${key}} = td, {length} = ${key}; i < length; i++){`, `const voice = ${options.applyTransformers ? `t_voiceStateUpdate(client_ptr, ${key}[i])` : `${key}[i]`};`, `await client_ptr.cache.set(${CacheElementType.VOICE_STATE}, \`\${voice.${vUid}}:\${voice.${vCid}}\`, voice);`, "}");
                }
                temp.push(`await client_ptr.cache.set(${CacheElementType.GUILD},`, options.applyTransformers ? "td.id, td" : "payload.d.id, {...payload.d,channels: undefined,threads:undefined,voice_states:undefined}", ");");
                this.#createListener(GatewayEvent.GuildCreate, "guildCreate", { when: enabledGuild.create, content: temp.join("") });
            }
            if (typeof enabledGuild?.update !== "undefined") this.#createListener(GatewayEvent.GuildUpdate, "guildUpdate", { when: enabledGuild.update, content: `await client_ptr.cache.set(${CacheElementType.GUILD}, ${options.applyTransformers ? "td.id, td" : "payload.d.id, payload.d"});` });
            if (typeof enabledGuild?.delete !== "undefined") this.#createListener(GatewayEvent.GuildDelete, "guildDelete", { when: enabledGuild.delete, content: `await client_ptr.cache.delete(${CacheElementType.GUILD}, ${options.applyTransformers ? "td.id" : "payload.d.id"});` });
            if (typeof enabledChannel?.create !== "undefined") this.#createListener(GatewayEvent.ChannelCreate, "channelCreate", { when: enabledChannel.create, content: `await client_ptr.cache.set(${CacheElementType.CHANNEL}, ${options.applyTransformers ? "td.id, td" : "payload.d.id, payload.d"});` });
            if (typeof enabledChannel?.update !== "undefined") this.#createListener(GatewayEvent.ChannelUpdate, "channelUpdate", { when: enabledChannel.update, content: `await client_ptr.cache.set(${CacheElementType.CHANNEL}, ${options.applyTransformers ? "td.id, td" : "payload.d.id, payload.d"});` });
            if (typeof enabledChannel?.delete !== "undefined") this.#createListener(GatewayEvent.ChannelDelete, "channelDelete", { when: enabledChannel.delete, content: `await client_ptr.cache.delete(${CacheElementType.CHANNEL}, ${options.applyTransformers ? "td.id" : "payload.d.id"});` });
            if (typeof enabledThreads?.create !== "undefined") this.#createListener(GatewayEvent.ThreadCreate, "threadCreate", { when: enabledThreads.create, content: `await client_ptr.cache.set(${CacheElementType.CHANNEL}, ${options.applyTransformers ? "td.id, td" : "payload.d.id, payload.d"});` });
            if (typeof enabledThreads?.update !== "undefined") this.#createListener(GatewayEvent.ThreadUpdate, "threadUpdate", { when: enabledThreads.update, content: `await client_ptr.cache.set(${CacheElementType.CHANNEL}, ${options.applyTransformers ? "td.id, td" : "payload.d.id, payload.d"});` });
            if (typeof enabledThreads?.delete !== "undefined") this.#createListener(GatewayEvent.ThreadDelete, "threadDelete", { when: enabledThreads.delete, content: `await client_ptr.cache.delete(${CacheElementType.CHANNEL}, ${options.applyTransformers ? "td.id" : "payload.d.id"});` });
            if (typeof options.enabled.self !== "undefined") {
                if (options.applyTransformers && !this.#callbacks.has("t_userUpdate")) {
                    if (typeof this.#transformers.userUpdate === "undefined") throw new Error("Missing 'userUpdate' transformer");
                    if (this.#transformers.userUpdate.return === TransformerReturnType.MULTIPLE) throw new Error("The transformer for 'userUpdate' should only return 1 value");
                    this.#callbacks.set("t_userUpdate", this.#transformers.userUpdate.handler);
                }
                this.#createListener(GatewayEvent.UserUpdate, "userUpdate", { when: options.enabled.self, content: `client_ptr.user = ${options.applyTransformers ? "td" : "payload.d"}` });
            }
        }
        return this;
    }

    public addListener<L extends Listeners<C, T> = Listeners<C, T>, K extends (keyof L) & string = (keyof L) & string>(name: K, handler: L[K] & {}, once = false): this {
        if (name === "setup") {
        ...
        this.#createListener(event, name);
        return this;
    }

    public getCompilationOutput(): { handlers: { names: Array<string>, callbacks: Array<(...args: any) => any> }, stack: string } {
        const hasReadyListener = this.#stack.get("ready") !== "";
        if (!hasReadyListener) this.#addReadyListener();
        return { handlers: { names: [...this.#callbacks.keys()], callbacks: [...this.#callbacks.values()] }, stack: [...this.#stack.values()].join("") };
    }
    ...
    public addListenersFromObject(listeners: Listeners<C, T>): this {
        for (const [event, handler] of Object.entries(listeners)) this.addListener(<never>event, <never>handler);
        return this;
    }
}
