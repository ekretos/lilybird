...
export class WebSocketManager {
    ...
    async #connect(url: string): Promise<void> {
        ...
        ws.addEventListener("message", (event) => {
            ...
            switch (payload.op) {
                case GatewayOpCode.Dispatch: this.#dispatch(payload); break;
                case GatewayOpCode.Hello: { this.#startTimer(payload.d.heartbeat_interval); if (this.#isResuming && this.#canResume()) this.#resume(); else { this.#isResuming = false; this.#identify(); } break; }
                case GatewayOpCode.Heartbeat: this.#sendHeartbeatPayload(); break;
                case GatewayOpCode.Reconnect: ws.close(1001); break;
                case GatewayOpCode.InvalidSession: this.#debug?.(DebugIdentifier.InvalidSession); if (payload.d === true && this.#canResume()) { this.#isResuming = true; ws.close(1001); } else { this.#isResuming = false; this.#sequenceNumber = null; this.#clearResumeInfo(); ws.close(1000); } break;
                case GatewayOpCode.HeartbeatACK: this.#gotACK = true; this.#heartbeatPending = false; this.#debug?.(DebugIdentifier.ACK); break;
                default: break;
            }
        });
    }
    ...
    public async ping(): Promise<number> { if (typeof this.#ws === "undefined" || typeof this.#ws.ping !== "function") throw new Error("WebSocket is not connected"); return new Promise((resolve, reject) => { const start = performance.now(); const timeout = setTimeout(() => reject(new Error("WebSocket ping timed out")), 5_000); this.#ws?.addEventListener("pong", () => { clearTimeout(timeout); resolve(performance.now() - start); }, { once: true }); this.#ws?.ping?.(); }); }
    public updatePresence(presence: UpdatePresenceStructure): void { if (typeof this.#ws === "undefined" || this.#ws.readyState !== WebSocket.OPEN) throw new Error("WebSocket is not connected"); const options: UpdatePresence = { op: GatewayOpCode.PresenceUpdate, d: presence, s: null, t: null }; this.#ws.send(JSON.stringify(options)); }
...
export interface ShardManagerOptions { token: string; intents: number; presence?: UpdatePresenceStructure; shardCount?: number; maxConcurrency?: number; reconnect?: boolean; }
export interface ShardDispatch { shardId: number; data: ReceiveDispatchEvent; }
export class ShardManager {
    readonly #options: ShardManagerOptions; readonly #dispatch: (event: ShardDispatch) => any; readonly #debug?: DebugFunction; readonly #shards = new Map<number, WebSocketManager>(); #gateway?: GetGatewayBotResponse;
    public constructor(options: ShardManagerOptions, dispatch: (event: ShardDispatch) => any, debug?: DebugFunction) { if (!options.token) throw new Error("No token was found"); if (!Number.isInteger(options.intents) || options.intents < 0) throw new Error("Invalid intents"); this.#options = options; this.#dispatch = dispatch; this.#debug = debug; }
    ...
    async #getGatewayBot(): Promise<GetGatewayBotResponse> { const response = await fetch("https://discord.com/api/v10/gateway/bot", { headers: { Authorization: `Bot ${this.#options.token}` } }); if (!response.ok) throw new Error("Unable to retrieve gateway information"); return await response.json() as GetGatewayBotResponse; }
}
