...
export class WebSocketManager {
    ...
    #resume(): void {
        ...
        const payload: Resume = { op: GatewayOpCode.Resume, d: { token: this.#options.token, session_id: this.resumeInfo.id, seq: this.#sequenceNumber as number }, s: null, t: null };
...
export class ShardManager {
    ...
    async #connectShard(id: number, count: number, url: string): Promise<void> {
        const manager = new WebSocketManager({ token: this.#options.token, intents: this.#options.intents, presence: this.#options.presence, reconnect: this.#options.reconnect, shard: [id, count] }, (data) => this.#dispatch({ shardId: id, data }), this.#debug);
