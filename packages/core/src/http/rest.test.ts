...
describe("REST reliability", () => {
    ...
    it("retries 429 responses using Retry-After", async () => {
        let calls = 0;
        globalThis.fetch = mock(async () => { calls++; if (calls === 1) return new Response(JSON.stringify({ code: 20028, message: "rate limited", retry_after: 0.001, global: false }), { status: 429, headers: { "Retry-After": "0.001" } }); return new Response(JSON.stringify({ ok: true }), { status: 200 }); });
    ...
    it("retries idempotent 5xx responses", async () => {
        let calls = 0;
        globalThis.fetch = mock(async () => { calls++; return calls === 1 ? new Response("", { status: 503 }) : new Response(JSON.stringify({ ok: true }), { status: 200 }); });
        const rest = new REST("token", { maxRetries: 1 });
        await expect(rest.makeAPIRequest("GET", "users/@me")).resolves.toEqual({ ok: true });
        expect(calls).toBe(2);
    });
    it("does not retry non-idempotent 5xx responses", async () => {
        let calls = 0;
        globalThis.fetch = mock(async () => { calls++; return new Response("", { status: 503 }); });
    ...
    it("wraps network failures", async () => {
        globalThis.fetch = mock(async () => { throw new Error("socket failure"); });
        const rest = new REST("token", { maxRetries: 0 });
        const error = await rest.makeAPIRequest("GET", "users/@me").catch((value) => value);
