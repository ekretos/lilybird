...
export class REST {
    ...
    public async makeAPIRequest<T>(
        ...
        reasonOrFiles?: string | Array<LilybirdAttachment>
    ): Promise<T> {
        const headers: Record<string, string> = {
            // eslint-disable-next-line @typescript-eslint/naming-convention
            "User-Agent": "DiscordBot (https://github.com/Didas-git/lilybird, 0.8.0)"
        };
        let body: FormData | string | undefined = undefined;

        if (typeof this.#token !== "undefined") headers.Authorization = `Bot ${this.#token}`;

        if (typeof data !== "undefined") {
            if (data instanceof FormData) {
                if (typeof reasonOrFiles === "string") headers["X-Audit-Log-Reason"] = reasonOrFiles;
                body = data;
            } else if (typeof reasonOrFiles !== "undefined" && typeof reasonOrFiles !== "string") {
                const { body: form, reason } = this.#handleFiles(data, reasonOrFiles);
        ...
        const result = await this.#request<T>(method, path, {
            method,
            headers,
            body
        });
    ...
    #handleFiles(data: Record<string, any>, files: Array<LilybirdAttachment>): { body: FormData, reason?: string } {
        const form = new FormData();
        const temp: Array<Partial<Channel.AttachmentStructure>> = [];
        let reason: string | undefined;

        if ("reason" in data && typeof data.reason === "string") {
            ({ reason } = data as { reason?: string });
            delete data.reason;
        }
        ...
        const dataRecord = data as { data?: { attachments?: Array<Partial<Channel.AttachmentStructure>> }, attachments?: Array<Partial<Channel.AttachmentStructure>> };
        if (typeof dataRecord.data !== "undefined") dataRecord.data.attachments = [...temp, ...dataRecord.data.attachments ?? []];
        else dataRecord.attachments = [...temp, ...dataRecord.attachments ?? []];
        form.append("payload_json", JSON.stringify(data));

        return { body: form, reason };
    }
    ...
    #backoff(attempt: number): number {
        return Math.min(1_000 * Math.pow(2, attempt), 10_000) + Math.floor(Math.random() * 250);
    }
